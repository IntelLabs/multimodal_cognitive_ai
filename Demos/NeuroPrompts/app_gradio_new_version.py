...
with grad.Blocks(css=css) as demo:
  ...
  with grad.Row():
    with grad.Column(scale=1):
      input_txt = grad.Textbox(label=" Input prompt", placeholder="Enter your prompt")
      with grad.Group():
        input_artist = grad.Dropdown(artists_list, label="Artist", allow_custom_value=True)
        input_style = grad.Dropdown(styles_list, label="Style", allow_custom_value=True)
        input_format = grad.Dropdown(formats_list, label="Format", allow_custom_value=True)
        input_perspective = grad.Dropdown(perspective_list, label="Perspective", allow_custom_value=True)
        input_booster= grad.Dropdown(booster_list, label="Booster", allow_custom_value=True)
        input_vibe = grad.Dropdown(vibe_list, label="Vibe", allow_custom_value=True)
      with grad.Group():
        input_negative = grad.Textbox(label="Negative constraints", info="Enter a comma-separated sequence of terms that should not appear in the optimized prompt")
      with grad.Accordion(label="Parameters", open=False) as og:
        input_lp = grad.Number(value=1.0, label = "Length penalty")
        input_max_length = grad.Number(value=77, label="Max length")
        input_beam_size = grad.Number(value=5, label="Beam size",)
        input_inference_steps = grad.Number(value=25, label="Stable Diffusion inference steps")
...
demo.launch(share=True, server_name=host)
