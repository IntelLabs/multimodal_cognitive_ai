...
from transformers import FlavaFeatureExtractor, FlavaImageModel, FlavaModel
...
# model = FlavaModel.from_pretrained("facebook/flava-full").eval().to(device)
...
def collate_fn_image(batch_list):
    ...
    # print(image_ids)
    batch = feature_extractor(images=images, return_tensors="pt").to(device)
    batch['image_paths'] = image_paths
    return batch

def compute_all_images_embeddings(path_to_model, coco_filepath):
    model = FlavaModel.from_pretrained(path_to_model).eval().to(device)
    ...
    try:
        for _, batch in enumerate(tqdm(dataloader)):    
            ...
            with torch.no_grad():
                outputs = model.get_image_features(**batch)
            batch_size = len(image_paths)
            for bidx in range(batch_size):
                image_path = image_paths[bidx]
                embeds_img   = outputs[bidx, 0, :]
                embeds_img /= embeds_img.norm(dim=-1, keepdim=True)
                embeds_img = embeds_img.cpu()
...
def collate_fn_caption(batch_list):
    ...
    batch = tokenizer(text=captions, return_tensors="pt", padding="max_length", max_length=77, truncation=True).to(device)
    ...
    try:
        for _, batch in enumerate(tqdm(dataloader)):    
            ...
            for bidx in range(batch_size):
                ...
                embeds_txt   = outputs[bidx, 0, :]
                embeds_txt /= embeds_txt.norm(dim=-1, keepdim=True)
                embeds_txt = embeds_txt.cpu()
...
def compute_topk_image_text_retrieval(image_extracted_embeddings, text_extracted_embeddings, topk_list, input_table):
    ...
    for i, image_path in tqdm(enumerate(image_extracted_embeddings.keys()), total=len(image_extracted_embeddings.keys())):
        ...
        values, indices = compute_similarity_scores(image_embedding, all_text_embeddings)
...
def compute_topk_text_image_retrieval(image_extracted_embeddings, text_extracted_embeddings, topk_list, input_table):
    ...
    for i, caption in tqdm(enumerate(text_extracted_embeddings.keys()),total=len(text_extracted_embeddings.keys())):
        ...
        values, indices = compute_similarity_scores(text_embedding, all_image_embeddings)
        for top_k in topk_list:
            candidate_indices = indices[:top_k]
...
def compute_similarity_scores(source_embedding, all_target_embeddings):
...
def evaluate_flava(path_to_model, coco_filepath):
    ...
    recall_image_retrieval = compute_topk_text_image_retrieval(image_extracted_embeddings, text_extracted_embeddings, top_ks, input_table)
    print('Computing image 2 text retrieval')
    recall_text_retrieval = compute_topk_image_text_retrieval(image_extracted_embeddings, text_extracted_embeddings, top_ks, input_table)
    return recall_image_retrieval, recall_text_retrieval

def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description='Evaluate Flava ',
        usage='Flava_evaluation.py [<args>] [-h | --help]'
    )
...
def main(args):
    ...
    if checkpoint_folder == 'baseline':
        ...
        res = {}
        res['model'] = name
        recall_image_retrieval, recall_text_retrieval = evaluate_flava(model_name, coco_filepath)
        res.update(recall_image_retrieval)
        res.update(recall_text_retrieval)
        all_results.append(res)
    else:
        name = 'finetuned_flava'
        for chkpt in tqdm(all_checkpoints, desc='processing each checkpoint'):
            print('processing checkpoints:', chkpt)
            res = {}
            res['model'] = chkpt
            recall_image_retrieval, recall_text_retrieval = evaluate_flava(chkpt, coco_filepath)
            res.update(recall_image_retrieval)
            res.update(recall_text_retrieval)
            all_results.append(res)
