...
for i in range(len(food101_labels)):
    tmp = food101_labels[i].lower()
    
    #replace '_' with space
    tmp = tmp.replace('_', ' ')    
    sents.append(f'a photo of {tmp}, a type of food')
    
def collate_fn(batch_list):
    images, labels = list(zip(*batch_list))
    batch = processor(images=images, text=sents, padding=True, return_tensors='pt', truncation=True).to(device)
    batch['labels'] = labels
    return batch


def evaluate_food101(path_to_model):
    model = CLIPModel.from_pretrained(path_to_model).to("cuda")    
    dataloader = DataLoader(food101_data, batch_size=64, shuffle=False, drop_last=False, collate_fn=collate_fn)
    all_labels = []
    all_predictions = []
    try:
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description='Evaluate Food101 test set for every checkpoint enclosed in a folder',
        usage='Food101EvaluationBatch.py [<args>] [-h | --help]'
    )
...
def main(args):
    ...
    for chkpt in tqdm(all_checkpoints, desc='processing each checkpoint'):
        ...
        res = {}
        res['model'] = chkpt
        accuracy, all_labels, all_predictions = evaluate_food101(chkpt)
