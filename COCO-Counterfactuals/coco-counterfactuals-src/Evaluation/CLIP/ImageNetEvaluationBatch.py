...
for i in range(len(imagenet_labels)):
    ...
    sents.append(f'a photo of a {tmp}')
...
def evaluate_imagenet(path_to_model):
    model = CLIPModel.from_pretrained(path_to_model).to("cuda")    
    dataloader = DataLoader(imagenet_data, batch_size=64, shuffle=False, drop_last=False, collate_fn=collate_fn)
    all_labels = []
    all_predictions = []
    try:
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description='Evaluate Imagenet val set for every checkpoint enclosed in a folder',
        usage='ImageNetEvaluationBatch.py [<args>] [-h | --help]'
    )
...
def main(args):
    ...
    for chkpt in tqdm(all_checkpoints, desc='processing each checkpoint'):
        ...
        res = {}
        res['model'] = chkpt
        accuracy, all_labels, all_predictions = evaluate_imagenet(chkpt)
