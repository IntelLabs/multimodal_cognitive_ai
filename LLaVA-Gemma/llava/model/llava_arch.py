...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_v2(
        ...
        if (input_ids == IMAGE_TOKEN_INDEX).sum() == 0:
            cur_input_embeds = self.get_model().embed_tokens(input_ids)
            return None, position_ids, attention_mask, past_key_values, cur_input_embeds, labels
        ...
        #     if num_images == 0:
        #         raise NotImplementedError
        #         print((input_ids == IMAGE_TOKEN_INDEX).sum(dim=1))
        #         cur_image_idx += 1
        #     else:
        ...
        return None, position_ids, attention_mask, past_key_values, input_embeds_clone, labels
    ...
    def prepare_inputs_labels_for_multimodal_pad(
        ...
        images_mask = input_ids == IMAGE_TOKEN_INDEX
        # print(images_mask.sum())
        input_ids[images_mask] = 0
        input_embeds = self.get_model().embed_tokens(input_ids)
        ...
        if position_ids is None:
            position_ids = torch.arange(0, labels.shape[1], dtype=torch.long, device=input_ids.device).unsqueeze(0)
        ...
        # image_indices = torch.where(input_ids == IMAGE_TOKEN_INDEX)
        # input_ids[image_indices] = 0
        # labels[image_indices] = IGNORE_INDEX
        # input_embeds = self.get_model().embed_tokens(input_ids)

        # final_input_embeds = input_embeds.clone()
        # final_input_embeds.retain_grad()

        # final_input_embeds[image_indices] = image_features.reshape(-1, final_input_embeds.shape[-1])
