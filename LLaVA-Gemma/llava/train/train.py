...
#
# First Copyright:
# Copyright (c) 2023 Rohan Taori, Ishaan Gulrajani, Tianyi Zhang, Yann Dubois, Xuechen Li
#
...
# ==============================================================================
"""
27 Feb 2024, Musashi Hinck - Making modifications for Gemma compatibility 
"""
...
def preprocess_gemma(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        # re_rounds = [conv.sep.join(rounds[:2])] # user + gpt
        re_rounds = []
        for conv_idx in range(0, len(rounds), 2):
            re_rounds.append(conv.sep.join(rounds[conv_idx:conv_idx+2]))    # user + gpt
        """
        Notes from Musashi on the logic

        So it works out despite being 2 slightly different things:

        - The first chunk is prepended by BOS
        - Subsequent chunks there is an <end_of_turn> token that is covered by the <BOS> (but the result is the same)
        - I've removed the -1 on the instruction_len because I want to mask the newline that is really part of the chat template.
        - If we want to have it write the chat template components as well, then that will take some different logic.

        Update 11 Mar 2024:
        - Making changes for new tokenizer
        """
        cur_len = 1 # skip BOS
        target[:cur_len] = IGNORE_INDEX
        for i, rou in enumerate(re_rounds):
            # MH: Split each round into user/assistant
            if rou == "": 
                break
            parts = rou.split(sep)
            if len(parts) != 2:
        ...
        if cur_len < tokenizer.model_max_length:
            if cur_len != total_len:
                ...
                # raise ValueError(
                #     f"warning: tokenization mismatch: {cur_len} vs. {total_len}."
                #     f"source conversation was {len(re_rounds)} rounds."
                #     f" (ignored)"
                # )
                print(
                    f"warning: tokenization mismatch: {cur_len} vs. {total_len}."
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            try:
                ...
                print(e)
                try:
                    print("Trying different file suffix")
                    image_file = image_file.split(".")[0] + (".png" if image_file.split(".")[-1]=='jpg' else ".jpg")
                    image = Image.open(os.path.join(image_folder, image_file)).convert('RGB')
                except FileNotFoundError as e:
                    print(f"Skipping to next item at index {i}.")
                    i += 1
                    if i > self.__len__():
                        i = 0
...
def train(attn_implementation=None):
    ...
    if IS_HPU:
        ...
        if local_rank_map:
            local_rank_map = local_rank_map.split(",")
            os.environ["HLS_MODULE_ID"] = local_rank_map[int(local_rank)] if len(local_rank_map) > 1 else local_rank_map[0]
        else:
            print("LOCAL_RANK_MAP environment variable is not set. This may cause errors if not using entire node")
    ...
    elif 'gemma' in model_args.model_name_or_path: # MH: tokenizer
        logger.info("Using Gemma tokenizer.")
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
            model_max_length=training_args.model_max_length,
    ...
    else: # MH: --version plain
        tokenizer.pad_token = tokenizer.unk_token # MH: Gemma has both pad and unk, still necessary?
    ...
    if model_args.vision_tower is not None:
        if IS_HPU: training_args.fsdp_config["xla_fsdp_v2"] = False 
