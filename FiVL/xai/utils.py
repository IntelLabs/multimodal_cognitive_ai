...
def model_generate(model_id, model, processor, tokenizer, question, image_path):
    ...
    if model_id == 'llava-hf/llava-1.5-7b-hf':
        ...
        generated_tokens= [processor.decode(i) for i in generated_token_ids]#processor.decode(generated_token_ids, skip_special_tokens=True)
...
def plot_heatmap2(mask, token, output_path, img_recover):
    fig, ax = plt.subplots()
