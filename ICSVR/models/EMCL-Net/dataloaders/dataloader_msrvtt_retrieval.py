...
class MSRVTTDataset(RetrievalDataset):
    ...
    def __init__(self, subset, anno_path, typ, video_path, tokenizer, max_words=32,
        ...
        super(MSRVTTDataset, self).__init__(subset, anno_path, typ, video_path, tokenizer, max_words,
    ...
    def _get_anns(self, subset='train'):
        ...
        csv_path = {'train': join(self.anno_path, 'MSRVTT_train.9k.csv'),
                    'val': join(self.anno_path, 'MSRVTT_' + self.typ + '.csv'),
                    'test': join(self.anno_path, 'MSRVTT_' + self.typ + '.csv')}[subset]
        print(csv_path)
        if exists(csv_path):
