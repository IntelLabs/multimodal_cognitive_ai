...
class MsvdDataset(RetrievalDataset):
    """MSVD dataset loader."""
    def __init__(self, subset, anno_path, typ, video_path, tokenizer, max_words=32,
                 max_frames=12, video_framerate=1, image_resolution=224, mode='all', config=None):
        super(MsvdDataset, self).__init__(subset, anno_path, typ, video_path, tokenizer, max_words,
    ...
    def _get_anns(self, subset='train'):
        ...
        print("########################################################################################")
        if(self.config.testtyp == 'part'):
            caption_file = os.path.join(self.anno_path, "MSVD_part_clip4clip_" + self.typ + ".pkl")
        else:
            caption_file = os.path.join(self.anno_path, "MSVD_clip4clip_" + self.typ + ".pkl")   
        
        print("Using Input File: {}".format(caption_file))
