...
import collections
import os
from Relational_Knowledge import utils

import numpy as np
import torch
from dataset.POS.process import read_common_pos
...
comp_list = set()
with open(os.environ["COMPARATIVE_WORDS_FILENAME"], "r") as f:
    for l in f:
        comp_list.add(" " + l.strip())
...
class ComparativeState:
    def __init__(self,
                 positive_trie: Trie,
                 state: List[Trie] = None,
                 met_phrases: RawConstraintList = None,cons_type=None,select_positive_phrases=False) -> None:

        self.root = positive_trie
        self.state = state if state else [self.root]
        self.met_phrases = met_phrases
        self.previous_sub_tokens = []
        self.select_positive_phrases = select_positive_phrases
        self.cons_type = cons_type
    ...
    def __str__(self):
        ...
        s += f']\nMet_phrases: {self.met_phrases}'
        return s
    def _start_new_word(self, word_id):
        if tokenizer.decode(word_id)[0] == " ":
            return True
        return False
    def _get_top_from_list(self,ranked_ids,top=5):
        seq_comp_list = list(comp_list)
        ids= [tokenizer.encode(x) for x in seq_comp_list]
        first = [x[0] for x in ids]
        # the rank of each comparative word respectively
        first_rank = [np.where(ranked_ids==x)[0][0] for x in first]
        # print(f"first_rank: {first_rank}")
        # the ranked rank of each comparative word
        first_rank_rank = np.argsort(first_rank)
        # print(f"first_rank_rank: {first_rank_rank}")
        return [seq_comp_list[x] for x in first_rank_rank[:top]]

    def set_pos_tokens(self, scores):
        if self.select_positive_phrases:
            return
        ranked_ids = np.argsort(scores)[::-1]
        top_selected = self._get_top_from_list(ranked_ids)
        self.select_positive_phrases = True
        # print(top_selected)
        encoded_top_selected = [tokenizer.encode(x) for x in top_selected]
        # print(encoded_top_selected)
        self.root = Trie(encoded_top_selected)
        self.state = [self.root]
        return Clause(idx=0, positive=encoded_top_selected, negative=[], min_count=1,
                                               max_count=1, count=0, satisfy=False, look_ahead=0)
    def allowed(self) -> Set[int]:
        allow = self.root.final().union(*[state.final() for state in self.state])
        allow |= set(self.root.children.keys()).union(*[set(state.children.keys()) for state in self.state])
        legal_words = set(x for x in allow if x < tokenizer.vocab_size)
    ...
    def advance(self, word_id_list: List[int]) -> 'PositiveState':
        ...
        new_word = self._start_new_word(word_id_list[0])
        if new_word:
            self.previous_sub_tokens = [word_id_list[0]]
        else:
            self.previous_sub_tokens.append(word_id_list[0])
        ...
        for word_id in word_id_list:
            for state in set(self.state + [self.root]):
                ...
                if word_id in state.final_ids:
                    met_phrases.append(state.trace_phrase(word_id))
        new_new_state, new_met_phrases = [], []
        # if met_phrases:
        if self.cons_type == "Comparative":
            decoded_id = tokenizer.decode(self.previous_sub_tokens)
            # pos = get_pos(tokenizer.decode(self.previous_sub_tokens)) # not working very well.
            # TODO use a list here
            # TODO make a suggestion to the comparative word for encouragment 
            # print(decoded_id,decoded_id in comp_list)
            if decoded_id in comp_list:
                new_met_phrases.append([word_id_list[0]])
                # print("xooooo",decoded_id)
                # print(self.root.final())
                # print(self.allowed())
                # new_new_state.append(new_state[idx])
            met_phrases = new_met_phrases
        # new_state = new_new_state
        if new_state:
            return ComparativeState(self.root, new_state, met_phrases if met_phrases else None, cons_type=self.cons_type)
        else:
            if len(self.state) == 1 and self.root == self.state[0] and not met_phrases:
                return self
            else:
                return ComparativeState(self.root, [self.root], met_phrases if met_phrases else None,cons_type=self.cons_type)
