...
def main():
    ...
    if args.model_name == 'gpt2-xl':
        ...
        model = AutoModelWithLMHead.from_pretrained(args.model_name, revision="float16", torch_dtype=torch.float16, low_cpu_mem_usage=True)
    ...
    for d in tqdm(data):
        for ins in d["prompt_and_constraint"]:
            ...
            if args.special_comparative_constraint:
                ...
                constraint_list_str.append( {'terms':["er","ier"]
, 'polarity': 1, 'max_count': 1, 'min_count': 1, 'type': args.special_comparative_constraint,'order' : max_order+1})
            ...
            flat_data.append({"input_id":tokenizer.encode(ins["input_id"], return_tensors="pt"),"constraint":constraints,'cur_constraints_str':constraint_list_str,'ent_class':d["ent_class"],'entity': d["entity"],'second_entity': d["second_entity"]})
    print(flat_data[0])
    general_constraints = []
    original_jsonl_objs = []
    ...
    with tqdm(total=len(flat_data)/args.batch_size) as pbar:
        for i in range(0,len(flat_data),args.batch_size):
            ...
            batch_input_ids = torch.cat(input_ids,dim=0).to("cuda")
            ...
            outputs = generate(self=model,
                    ...
                    do_beam_sample=args.do_beam_sample, beam_temperature=args.beam_temperature,
                            diversity=args.diversity,
                            return_all=args.return_all,length_penalties=length_penalties, lp_return_size=args.lp_return_size,
            ...
            for j in range(len(input_ids)):
                ...
                for z in range(out_seqs.shape[0]):
                    ...
                    generated_knowledge = decoded.split('<|endoftext|>')[0].rstrip()
                    generated_knowledge = generated_knowledge.strip().replace('<|endoftext|>', '')
