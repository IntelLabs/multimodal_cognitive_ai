...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if self.prompt_to_prompt and is_self_attn:
            ...
            sims = sim.chunk(x.size(0))
            # sim = torch.cat(tuple(itertools.chain.from_iterable([(sims[i], sims[i]) for i in range(0, x.size(0), 2)]))) # this works for pairwise batching
            n_captions = int(x.size(0)/2)
            sim = torch.cat(tuple([(sims[0])] * n_captions + [(sims[n_captions])] * n_captions))
